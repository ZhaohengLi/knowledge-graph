...
class TransEModel(object):
    ...
    def __init__(self, config):
        ...
        if config.L1_flag:  # use L1-norm
            # TO DO
            pos_f = tf.norm(pos_h_e + pos_r_e - pos_t_e, 1, axis=1)
            neg_f = tf.norm(neg_h_e + neg_r_e - neg_t_e, 1, axis=1)

        else:  # use L2-norm
            # TO DO
            pos_f = tf.norm(pos_h_e + pos_r_e - pos_t_e, 2, axis=1)
            neg_f = tf.norm(neg_h_e + neg_r_e - neg_t_e, 2, axis=1)

        with tf.name_scope("output"):
            # TO DO
            self.loss = tf.reduce_sum(tf.maximum(0.0, margin + pos_f - neg_f))
